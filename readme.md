...
进度双重同步：确保上方文字（如 1218 / 2299）与下方日志窗口的进度严格一致，消除用户视觉偏差。（尚未实现）

2.3 完整文件结构 (File Structure)
/m3u8-web-tool
├── index.html          # 主界面结构 (包含左侧步骤栏与右侧功能区)
├── style.css           # 包含流光进度条动画与响应式布局
├── ui-controller.js    # 负责：DOM操作、进度显示、统计面板解析
├── video-core.js       # 负责：FFmpeg初始化、VFS管理、分段处理逻辑
├── ffmpeg-core.js      # FFmpeg 静态资源 (由 initCore 加载)
├── ffmpeg-core.wasm    # 核心引擎 (31.2MB)
└── ffmpeg-core.worker.js # 多线程支持文件
